UI.buildResources = function() {
    ...
    if(!c) return; 
    c.innerHTML = "<strong>Ressources</strong><br><br>";
    ...
    for(let k in GeneratorDB) {
        if(GeneratorDB[k].unlocked) {
            ...
            for(let r in GeneratorDB[k].baseCost) {
                txt += `<span id="cost-${k}-${r}">${getCost(k,r)}</span> ${ResourceDB[r] ? ResourceDB[r].name.substring(0,3) : r} `;
            }
    ...
    for(let k in UpgradeDB) {
        if(UpgradeDB[k].unlocked && !UpgradeDB[k].purchased) {
            ...
            for(let r in UpgradeDB[k].cost) {
                txt += `${UpgradeDB[k].cost[r]} ${ResourceDB[r] ? ResourceDB[r].name.substring(0,3) : r} `;
            }
...
UI.updateGeneratorsScreen = function() {
    for(let k in GeneratorDB) {
        ...
        if(el) { 
            let aff = true; 
            for(let r in GeneratorDB[k].baseCost) {
                if(!ResourceDB[r] || ResourceDB[r].amount < getCost(k,r)) aff = false;
            }
            el.disabled = !aff; 
        }
    }
