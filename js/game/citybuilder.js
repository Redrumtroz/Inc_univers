const CityBuilder = {
    ...
    PALETTE: {
        grass: '#0f172a', path: '#334155', plazaBase: '#1e293b', roadUrban: '#0f172a',
        wallLight: '#e2e8f0', wallDark: '#cbd5e1', roofLight: '#3b82f6', roofDark: '#1d4ed8',
        woodLight: '#8a6b4e', woodDark: '#6a5035', stoneLight: '#94a3b8', stoneDark: '#64748b', tree: '#22c55e'
    },
    ...
    initUI() {
        let tb = document.getElementById('build-toolbar');
        if(!tb) return;
        tb.innerHTML = "";
        
        for(let k in BuildingDB) {
            if(!BuildingDB[k].unlocked) continue;
            let cst = ""; for(let r in BuildingDB[k].cost) { cst += BuildingDB[k].cost[r] + " " + ResourceDB[r].name.substring(0,2) + " "; }
            tb.innerHTML += `<button class="cb-btn ${this.selectedTool===k ? 'selected':''}" id="cb-btn-${k}" onclick="CityBuilder.selectTool('${k}')">
                <canvas class="cb-icon" id="cb-icon-${k}" width="45" height="45"></canvas>
                <span class="cb-label">${BuildingDB[k].name}</span>
                <span class="cb-cost">${cst}</span>
            </button>`;
        }
        
        // Tree & Remove
        tb.innerHTML += `<button class="cb-btn ${this.selectedTool==='tree' ? 'selected':''}" onclick="CityBuilder.selectTool('tree')">
            <canvas class="cb-icon" id="cb-icon-tree" width="45" height="45"></canvas>
            <span class="cb-label">Arbre</span><span class="cb-cost">Gratuit</span>
        </button>`;
        tb.innerHTML += `<button class="cb-btn ${this.selectedTool==='remove' ? 'selected':''}" style="border-color:#ef4444;" onclick="CityBuilder.selectTool('remove')">
            <div style="font-size:20px; margin-bottom: 2px;">🗑️</div>
            <span class="cb-label" style="color:#ef4444;">Détruire</span>
        </button>`;
        
        // Timeout pour laisser le DOM rendre les canvas
        setTimeout(() => this.generateUIcons(), 50);
    },

    selectTool(type) {
        this.selectedTool = type;
        document.querySelectorAll('.cb-btn').forEach(b => b.classList.remove('selected'));
        let btn = document.getElementById(`cb-btn-${type}`);
        if(btn) btn.classList.add('selected');
        // fallback pour tree/remove s'ils n'ont pas l'id formel :
    ...
    addEntity(x, y, type) {
        ...
        CityData.entities.push({
            ...
            details: { hasChimney: Math.random() > 0.4, roofVariant: Math.random() > 0.7 ? '#1e3a8a' : null, leafVariant: Math.random() > 0.7 ? '#15803d' : this.PALETTE.tree, props: props }
        });
    ...
    updateStats() {
        ...
        let elHap = document.getElementById('ui-happy'); 
        if(elHap) {
            elHap.innerText = CityData.happiness > 5 ? "Heureux" : (CityData.happiness < -5 ? "Mécontent" : "Neutre");
            elHap.style.color = CityData.happiness > 5 ? "#22c55e" : (CityData.happiness < -5 ? "#ef4444" : "#94a3b8");
        }
    ...
    drawHouse(cx, t = 0, d = {}) {
        let rD = d.roofVariant || this.PALETTE.roofDark, rL = d.roofVariant ? '#60a5fa' : this.PALETTE.roofLight;
    ...
    loop(timestamp) {
        ...
        allRender.forEach(e => {
            ...
            if (e === this.hoveredEntity && !this.isDragging && !e.type.startsWith('prop_')) this.ctx.filter = 'brightness(1.2) drop-shadow(0px 0px 8px rgba(255,255,255,0.8))';
