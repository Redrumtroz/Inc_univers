const Game = {
    ...
    async init() {
        ...
        UI.buildUpgrades(); 
        UI.buildSkills(); 
        if (window.TD && TD.init) TD.init(); 
        
        setTimeout(() => {
            if(document.getElementById('worldCanvas') && window.CityBuilder) {
                CityBuilder.init();
            }
        }, 150);
    ...
    buyUpgrade(id) {
        ...
        if(aff) {
            ...
            if(id==="axes") this.clickPower*=2; if(id==="market_unlock") { BuildingDB.market.unlocked = true; if(window.CityBuilder) CityBuilder.initUI(); }
    ...
    tick() {
        ...
        if (window.TD && TD.tick) TD.tick(); 
        UI.updateScreen();
        if(window.CityBuilder && CityBuilder.updateStats) CityBuilder.updateStats();
    }
};
